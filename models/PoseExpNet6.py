...
class PoseExpNet6(nn.Module):

    def __init__(self, nb_ref_imgs=2, output_exp=False):
        super(PoseExpNet6, self).__init__()
        ...
        if self.output_exp:
            upconv_planes = [256, 256, 256, 128, 64, 32, 16]
            ...
            self.predict_mask6 = nn.Conv2d(upconv_planes[1], self.nb_ref_imgs, kernel_size=3, padding=1)
            self.predict_mask5 = nn.Conv2d(upconv_planes[2], self.nb_ref_imgs, kernel_size=3, padding=1)
            self.predict_mask4 = nn.Conv2d(upconv_planes[3], self.nb_ref_imgs, kernel_size=3, padding=1)
            self.predict_mask3 = nn.Conv2d(upconv_planes[4], self.nb_ref_imgs, kernel_size=3, padding=1)
            self.predict_mask2 = nn.Conv2d(upconv_planes[5], self.nb_ref_imgs, kernel_size=3, padding=1)
            self.predict_mask1 = nn.Conv2d(upconv_planes[6], self.nb_ref_imgs, kernel_size=3, padding=1)
    ...
    def forward(self, target_image, ref_imgs):
        ...
        if self.output_exp:
            out_upconv7 = self.upconv7(out_conv7  )[:, :, 0:out_conv6.size(2), 0:out_conv6.size(3)]
            out_upconv6 = self.upconv6(out_upconv7)[:, :, 0:out_conv5.size(2), 0:out_conv5.size(3)]
            out_upconv5 = self.upconv5(out_upconv6)[:, :, 0:out_conv4.size(2), 0:out_conv4.size(3)]
            ...
            exp_mask6 = sigmoid(self.predict_mask6(out_upconv6))
            exp_mask5 = sigmoid(self.predict_mask5(out_upconv5))
        ...
        if self.training:
            return [exp_mask1, exp_mask2, exp_mask3, exp_mask4, exp_mask5, exp_mask6], pose
        else:
